...
export const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    decisions: [
      ...
      {
        heading: "7. (Proposed) A PO Intake Agent — free text and spreadsheets in, a governed purchase order out",
        body: "The manual-intake problem this module solves for search and stock visibility still exists one layer up, in two shapes. A contractor's order arrives as free text — phone, WhatsApp or email, no structure — and gets re-typed into Tally by hand. And when a distributor is the one shipping a bulk order, the source is often their own Excel sheet — rows of SKU, quantity and unit already tabular, just never mapped against the canonical catalogue above. The proposal reuses Griffy's Atlas agent pattern (typed extraction — BookingIntent/IntentProposal on the demand side today, POIntent for this case) against either source: parsing prose or parsing spreadsheet rows both resolve to the same structured line items — matched SKU, quantity, unit — each carrying the model's own confidence rather than a guess presented as fact. A line below the confidence threshold, an unmapped SKU, or a stock check that doesn't clear becomes an exception routed to a human, not silently dropped or auto-corrected — the same 'never invent a supplier or product' rule this module already enforces on catalogue search. The governance isn't bolted on afterward: Atlas's ToolRegistry already declares `mutates` per tool, and AtlasOrchestrator.invokeGuarded already refuses to run a mutating tool and returns ConfirmationRequired regardless of what an agent's catalogue claims — a rule formalized as RFC-001, approved and live in production since August 2026. So a PO Intake Agent could extract and propose a purchase order today — from a WhatsApp message or an uploaded Excel sheet — under the exact rule that already holds for every Atlas agent: it stays unable to write one until a human explicitly approves, without a single new governance mechanism being built. Still proposed, not built: no POIntent schema, no PO Intake Agent entry exists in the Atlas agent catalogue yet.",
  ...
  },
  {
    ...
    product: {
      ...
      body: [
        ...
        "Live in production since August 2026: POST /atlas/intent turns natural language into a typed, read-only booking intent, behind a provider layer (Anthropic and Gemini, chosen by config) and a typed tool registry wrapping four existing read-only endpoints — search_professionals, search_materials, check_availability, estimate_project_cost. The agent catalogue has since shipped too: GET /atlas/agents lists five agents — concierge, discovery, materials, estimator, planner — and POST /atlas/chat routes a conversation to one of them, each restricted to the tools its own definition names.",
        "That's the demand side of the same planner: a homeowner or contractor asking Atlas to find and book a consultation, or buy materials, is the concierge, discovery and materials agents in the existing catalogue doing exactly what they were built for. The Griffy Supply case study proposes the supply-side counterpart — a PO Intake Agent for the moment a distributor is the one submitting structured or semi-structured input, fulfilling or shipping a bulk order, rather than a customer asking a question.",
      ],
    },
    platform: [
